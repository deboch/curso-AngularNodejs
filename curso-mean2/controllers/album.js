...
function getAlbums(req,res){
	...
	if(!artistId){
		...
		//Sacar los albums de un artista conncreto
		var find = Album.find({artist: artistId})
	}
	...
	Album.find({artist: albumId}).remove((err, albumRemoved)=>{
		if(err){
			...
			if(!albumRemoved){
				...
				Song.find({album : albumRemoves._id}).remove((err,songRemoved) =>{
					if(err){
						res.status(500).send({message: 'Erroral eliminar la cancion'});
					}else{
						if(!songRemoved){
							res.status(404).send({message: 'Error al eliminar la cancion'});
						}else{
							res.status(200).send({album: artistRemoved});
						}
					}
				});
			}
